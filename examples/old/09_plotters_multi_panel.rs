...
fn parse_time(date: &str) -> NaiveDate {
    ...
    // 2019-04-25 00:00:00.000
    let _d = NaiveDate::parse_from_str(date, "%Y-%m-%d").unwrap();

    // dummy time 00:00:00.000
    let _time = NaiveTime::from_hms_milli_opt(0, 0, 0, 0).unwrap();
    let _dt = NaiveDateTime::new(_d, _time);
    Local.from_local_datetime(&_dt).unwrap().date_naive()
}

const OUT_FILE_NAME: &str = "plotters-doc-data/09_stock.png";
...
fn display_vec(v:Vec<(&'static str, f32, f32, f32, f32)>){

    println!("{v:?}");

    let numbers = NumVec(v);
    println!("{}", numbers);
...
struct NumVec(Vec<(&'static str, f32, f32, f32, f32)>);
