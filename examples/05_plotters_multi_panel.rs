...
fn main() {
    let root_area = BitMapBackend::new("images/time_series_chart.png", (600, 400)).into_drawing_area();
    root_area.fill(&WHITE).unwrap();

    let start_date = Utc.ymd(2019, 10, 1);
    let end_date = Utc.with_ymd_and_hms(2019, 10, 18, 0, 0, 0).unwrap(); //Utc.with_ymd_and_hms(2019, 10, 18);
